...
var testAbiChangedConfig = transformer.EventTransformerConfig{
	...
	Topic:               test_data.AbiChangedSignature,
...
var _ = Describe("AbiChanged Transformer", func() {
	XIt("fetches and transforms a AbiChanged event from mainnet chain", func() {
		...
		var dbResult []abi_changed.AbiChangedModel
		err = db.Select(&dbResult, `SELECT resolver, node, content_type FROM ens.abi_changed`)
		Expect(err).NotTo(HaveOccurred())
	...
	XIt("rechecks header for abi_changed event", func() {
		...
		var abi_changedChecked []int
		err = db.Select(&abi_changedChecked, `SELECT abi_changed_checked FROM public.checked_headers WHERE header_id = $1`, headerID)
		Expect(err).NotTo(HaveOccurred())
