...
var testNewOwnerConfig = transformer.TransformerConfig{
...
var _ = Describe("NewOwner Transformer", func() {
	It("fetches and transforms a NewOwner event from mainnet chain", func() {
		...
		Expect(res.Label).To(Equal(""))
		Expect(res.Owner).To(Equal(""))
		Expect(res.Subnode).To(Equal(""))
	})
	...
	It("unpacks an event log", func() {
		...
		entity := &new_owner.NewOwnerEntity{}

		var eventLog = test_data.EthNewOwnerLog

		err = contract.UnpackLog(entity, "NewOwner", eventLog)
		Expect(err).NotTo(HaveOccurred())
