...
var _ = Describe("Transfer Converter", func() {
	...
	Describe("ToEntity", func() {
		It("converts an eth log to a TestTransfer entity", func() {
			...
			entity := entities[0]
			Expect(entity).To(Equal(test_data.TransferEntity))
		})

		It("returns an error if converting log to entity fails", func() {
			_, err := converter.ToEntities("error abi", []types.Log{test_data.EthTransferLog})
	...
	Describe("ToModel", func() {
		var emptyEntity = transfer.TransferEntity{}

		It("converts an Entity to a Model", func() {
			models, err := converter.ToModels([]interface{}{test_data.TransferEntity})

			Expect(err).NotTo(HaveOccurred())
			Expect(len(models)).To(Equal(1))
			model := models[0]
			Expect(model).To(Equal(test_data.TransferModel))
		})
		...
		It("returns an error if the entity type is wrong", func() {
			...
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("entity of type test_data.WrongEntity, not transfer.TransferEntity"))
		})
